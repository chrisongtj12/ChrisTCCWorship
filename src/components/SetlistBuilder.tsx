...
export function SetlistBuilder({ songs, set, onChange }: Props) {
  ...
  const move = (i: number, dir: -1 | 1) => {
    const j = i + dir;
    if (j < 0 || j >= set.entries.length) return;
      ...
      {set.entries.length === 0 ? (
        ...
        <ol className="space-y-3">
          {set.entries.map((entry, i) => {
            ...
            return (
              <li key={i} className="rounded-xl bg-white p-3 shadow-sm dark:bg-slate-900">
                <div className="flex items-start gap-2">
                  <div className="flex flex-col pt-1">
                    <button onClick={() => move(i, -1)} aria-label="move up" className="text-slate-400 hover:text-sky-600">
                      ↑
                    </button>
                    <button onClick={() => move(i, 1)} aria-label="move down" className="text-slate-400 hover:text-sky-600">
                      ↓
                    </button>
                  </div>
